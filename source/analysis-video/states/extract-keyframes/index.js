...
class StateExtractKeyframes extends BaseState {
  async process() {
  ...
  sanityCheck() {
    ...
    if (!src.bucket || !src.key || !src.keyframesJson
      || src.startIndex === undefined || src.framesPerSlice === undefined) {
      throw new Error('missing bucket, key, keyframesJson, startIndex, and framesPerSlice');
    }
  }
  ...
  async extractKeyframes() {
    ...
    try {
      ...
      // upload key frames
      const prefix = PATH.parse(src.keyframesJson).dir;
      let processed = 0;
