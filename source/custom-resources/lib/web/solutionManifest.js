...
class SolutionManifest extends mxBaseResponse(class {}) {
  ...
  sanityCheck(data) {
    ...
    if (!(data.StateMachine.Training || {}).Name) {
      throw new Error('missing StateMachine.Training.Name');
    }
    if (!(data.StateMachine.Analysis || {}).Name) {
      throw new Error('missing StateMachine.Analysis.Name');
    }
  }
